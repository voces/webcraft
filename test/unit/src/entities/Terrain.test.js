...
export default () => describe( "Terrain", () => {
	...
	it( "constructor( <{ <units> }> )", () => {
		...
		assert.deepEqual( new Terrain( terrain1Props ), Object.assign( { units: new Collection() }, terrain1Props ) );

		const terrain2Units = new Collection();
		assert.equal( new Terrain( { units: terrain2Units } ).units, terrain2Units );

	} );

	it( "selectUnitsBoundedByRect( rect )", () => {

		const terrain = new Terrain( { units: [ { x: 0, y: 0 } ] } );
