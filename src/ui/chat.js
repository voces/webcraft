...
const chatInputSuggestion = document.getElementById( "chat-input-suggestion" );
const commands = [];
...
const onCommandInput = text => {
	...
	for ( const command of commands )
		if ( command.name.toLowerCase().startsWith( firstWord ) ) {
			...
			const args = command.args && command.args.slice( words.length - 1 ).map( a => a.required ? `<${a.name}>` : `[${a.name}]` ).join( " " );
...
const onCommandEnter = text => {
	...
	for ( const command of commands )
		if ( command.name.toLowerCase() === firstWord ) {
			...
			if ( ! command.args || words.length > command.args.filter( a => a.required ).length ) {
...
export const registerCommand = ( { name, aliases, args, comment, handler } ) => {

	const existingCommand = commands.find( c =>
		c.name === name ||
		c.aliases && c.aliases.includes( name ) ||
		aliases && aliases.includes( c.name ) ||
		c.aliases && aliases && c.aliases.some( a => aliases.includes( a ) )
	);
