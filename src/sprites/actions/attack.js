...
const isInRange = ( attacker, target ) => {
	...
	const distanceToTarget = Math.sqrt( ( target.x - attacker.x ) ** 2 + ( target.y - attacker.y ) ** 2 );
	return distanceToTarget < attacker.weapon.range + attacker.radius + target.radius;

};

export default ( attacker, target ) => {
	...
	const recalcPath = ( { x, y } ) => {
		...
		path = tweenPoints( pathingMap.withoutEntity( target, () =>
	...
	if ( attacker.speed ) {
		...
		attacker.action.render = delta => {
			...
			const range = attacker.weapon.range + attacker.radius + target.radius;
			...
			const pos = attacker.renderedPosition || attacker;
			const renderedDistanceToTarget = Math.sqrt(
				( target.x - pos.x ) ** 2 + ( target.y - pos.y ) ** 2
			);
			if ( renderedDistanceToTarget < range )
				return;
			...
			attacker.renderedPosition = { x, y };
			attacker.elem.style.left = ( x - attacker.radius ) * WORLD_TO_GRAPHICS_RATIO + "px";
			attacker.elem.style.top = ( y - attacker.radius ) * WORLD_TO_GRAPHICS_RATIO + "px";
	...
	attacker.action.update = delta => {
		...
		if ( isInRange( attacker, target ) ) {
			...
			// Not on cooldown
			if ( ! attacker.weapon.last || attacker.weapon.last + attacker.weapon.cooldown < game.round.lastUpdate ) {

				if ( attacker.weapon.projectile === "instant" ) {

					const damage = attacker.isMirror ? 0 : attacker.weapon.damage;
					const actualDamage = target.damage( damage );
					if ( attacker.weapon.onDamage )
						attacker.weapon.onDamage( target, actualDamage, damage );
				...
				game.round.setTimeout(
					() => attacker.elem && attacker.elem.classList.remove( "attack" ),
					0.250
				);
				attacker.weapon.last = game.round.lastUpdate;
