...
class Sprite implements Emitter<SpriteEvents> {
	...
	constructor( {
		...
		if ( ! round ) throw new Error( "trying to create a sprite outside a round" );
		this.round = round;
		...
		// Display
		this.elem = Object.assign( document.createElement( "div" ), { sprite: this } );
	...
	damage( amount: number ) {
		...
		const ignoreArmor = this.buildProgress === undefined || this.buildProgress < 1;
	...
	set health( value ) {
		...
		if ( this._health )
			this.elem.style.opacity = Math.max( this._health / this.maxHealth, 0.1 ).toString();
