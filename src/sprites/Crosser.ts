...
export default class Crosser extends Unit {
	...
	static isCrosser = ( sprite: Crosser | Sprite ): sprite is Crosser => sprite instanceof Crosser
	...
	buildAt( target: Point, ObstructionClass: ObstructionSubclass ): void {
		...
		const blueprint = this.owner === game.localPlayer ? new Blueprint( { ...target, radius: ObstructionClass.defaults.radius } ) : undefined;
			...
			update: delta => {
				...
				if ( actualDistance < BUILD_DISTANCE ) {
					...
					if ( ObstructionClass.defaults.cost ) {
						...
						const check = game.localPlayer.checkResources( ObstructionClass.defaults.cost );
						if ( check?.length ) {
						...
						game.localPlayer.subtractResources( ObstructionClass.defaults.cost );
					...
					this.round.pathingMap.withoutEntity( this, () => {
						...
						if ( this.round.pathingMap.pathable( obstruction, target.x, target.y ) ) {
						...
						this.setPosition( this.round.pathingMap.nearestSpiralPathing( x, y, this ) );
