...
export abstract class Obstruction extends Sprite {
	...
	static isObstruction = ( sprite: Obstruction | Sprite ): sprite is Obstruction => sprite instanceof Obstruction
	...
	constructor( {
		...
		this.action = {
			update: delta => {
				...
				renderProgress = this.buildProgress = Math.min( this.buildProgress + delta / this.buildTime, 1 );
			...
			render: delta => {
				...
				renderProgress = Math.min( renderProgress + delta / this.buildTime, 1 );
				...
				this.elem.style.opacity = ( renderedHealth / this.maxHealth ).toString();
