...
class Tile {
	...
	constructor( xTile: number, yTile: number, xWorld: number, yWorld: number, pathing: Pathing ) {
...
interface Cache {
	_linearPathable: ( ...args: Parameters<typeof Tilemap.prototype._linearPathable> ) => ReturnType<typeof Tilemap.prototype._linearPathable>;
	_pathable: ( ...args: Parameters<typeof Tilemap.prototype._pathable> ) => ReturnType<typeof Tilemap.prototype._pathable>;
	pointToTilemap: ( ...args: Parameters<typeof Tilemap.prototype.pointToTilemap> ) => ReturnType<typeof Tilemap.prototype.pointToTilemap>;
}
...
export default class Tilemap {
	...
	constructor( { pathing, resolution = DEFAULT_RESOLUTION, layers }: {pathing: Pathing[][], resolution?: number, layers?: number[][]} ) {
	...
	_pathable( map: Footprint, xTile: number, yTile: number, test?: ( tile: Tile ) => boolean ): boolean {
	...
	nearestPathing( xWorld: number, yWorld: number, entity: Entity, test?: ( tile: Tile ) => boolean ): Point {
		...
		const offset = ( point: Point ) => ( { x: point.x + radiusOffset, y: point.y + radiusOffset } );

		// Create our heap
		const distance = ( a: Point, b: Point ) => ( b.x - a.x ) ** 2 + ( b.y - a.y ) ** 2;
	...
	path( entity: Entity, target: Point ): Point[] {
		...
		const h = ( a: Point, b: Point ) => Math.sqrt( ( b.x - a.x ) ** 2 + ( b.y - a.y ) ** 2 );

		const startHeap = new BinaryHeap( ( node: Tile ) => node.__startRealPlusEstimatedCost ?? 0 );
		...
		const endHeap = new BinaryHeap( ( node: Tile ) => node.__endRealPlusEstimatedCost ?? 0 );
		...
		while ( startHeap.length ) {
			...
			for ( let i = 0, length = startNeighbors.length; i < length; i ++ ) {
				...
				if ( startCurrent.__startParent && cache._linearPathable( entity, startCurrent.__startParent, neighbor, cache ) ) {
					...
					const gScore = startCurrent.__startParent.__startRealCostFromOrigin ?? 0 + h( startCurrent.__startParent, neighbor );
					// First visit or better score than previously known
					if ( ! neighbor.__startVisited || gScore < ( neighbor.__startRealCostFromOrigin ?? 0 ) ) {
						...
						if (
							neighbor.__startEstimatedCostRemaining < ( startBest.__startEstimatedCostRemaining ?? 0 ) ||
							neighbor.__startEstimatedCostRemaining === startBest.__startEstimatedCostRemaining &&
							neighbor.__startRealCostFromOrigin < ( startBest.__startRealCostFromOrigin ?? 0 )
						) {
			...
			for ( let i = 0, length = endNeighbors.length; i < length; i ++ ) {
				...
				if ( endCurrent.__endParent && cache._linearPathable( entity, endCurrent.__endParent, neighbor, cache ) ) {
					...
					const gScore = endCurrent.__endParent.__endRealCostFromOrigin ?? 0 + h( endCurrent.__endParent, neighbor );
					// First visit or better score than previously known
					if ( ! neighbor.__endVisited || gScore < ( neighbor.__endRealCostFromOrigin ?? 0 ) ) {
						...
						if (
							neighbor.__endEstimatedCostRemaining < ( endBest.__endEstimatedCostRemaining ?? 0 ) ||
							neighbor.__endEstimatedCostRemaining === endBest.__endEstimatedCostRemaining &&
							neighbor.__endRealCostFromOrigin < ( endBest.__endRealCostFromOrigin ?? 0 )
						) {
		...
		if ( debugging ) {
			...
			const max = this.grid.reduce( ( max, row ) =>
				row.reduce(
					( max, cell ) => Math.max(
						max,
						cell.__startTag === startTag && cell.__startVisited ?
							cell.__startRealPlusEstimatedCost ?? 0 :
							cell.__endTag === endTag && cell.__endVisited ?
					...
					if ( this.grid[ y ][ x ].__startTag === startTag && this.grid[ y ][ x ].__startVisited || this.grid[ y ][ x ].__endTag === endTag && this.grid[ y ][ x ].__endVisited )
						placeTile(
							...
							( this.grid[ y ][ x ].__startTag === startTag ?
								...
								this.grid[ y ][ x ].__endRealPlusEstimatedCost ) ?? 0 - min / d,
						);
	...
	_linearPathable( entity: Entity, startTile: Point, endTile: Point, cache: Cache = this ): boolean {
	...
	paint(): void {
		...
		const host = this._elem || ( this._elem = ( () => {
	...
	paintMap( map: Footprint, xTile: number, yTile: number ): void {
		...
		for ( let y = yTile + map.top; y < yTile + map.height + map.top; y ++ )
			for ( let x = xTile + map.left; x < xTile + map.width + map.left; x ++, i ++ ) {
				...
				cell.setAttribute( "grid", ( this.grid[ y ] === undefined ? "no-y" : this.grid[ y ][ x ] === undefined ? "no-x" : this.grid[ y ][ x ].pathing ).toString() );
