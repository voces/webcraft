...
const distanceBetweenPoints = ( { x: x1, y: y1 }: Point, { x: x2, y: y2 }: Point ) =>
...
export const tweenPoints = ( points: Point[] ): PathTweener => {
	...
	for ( let i = 0; i < points.length; i ++ ) {
		...
		if ( i < points.length - 1 ) {
			...
			annotatedPoints[ i ].end = ( annotatedPoints[ i ].start ?? 0 ) +
				distanceBetweenPoints( points[ i ], points[ i + 1 ] );
			annotatedPoints[ i ].distance = ( annotatedPoints[ i ].end ?? 0 ) - ( annotatedPoints[ i ].start ?? 0 );
	...
	let curPoint = 0;
	const func = points.length > 1 ? ( progress: number ) => {
