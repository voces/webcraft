...
describe("BinaryHeap#bubbleUp", () => {
	it("moves an element up to its proper heap order", () => {
		...
		expect([...heap]).toEqual([
			1, 2, 7, 4, 3, 10, 8, 11, 5, 12, 13, 19, 15, 16, 9, 20, 14, 17, 6,
			18,
		]);
		...
		expect([...heap]).toEqual([
			0, 1, 7, 4, 2, 10, 8, 11, 5, 3, 13, 19, 15, 16, 9, 20, 14, 17, 6,
			18, 12,
		]);
	});
});
...
describe("BinaryHeap#sinkDown", () => {
	it("moves an element down to its proper heap order", () => {
		...
		expect([...heap]).toEqual([
			0, 1, 6, 3, 2, 9, 7, 10, 4, 11, 12, 18, 14, 15, 8, 19, 13, 16, 5,
			17,
		]);
		...
		expect([...heap]).toEqual([
			0, 3, 1, 6, 4, 2, 9, 7, 10, 5, 11, 12, 18, 14, 15, 8, 19, 13, 16,
			20, 17,
		]);
	});
});
