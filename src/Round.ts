...
class Round {
	...
	constructor( { time, settings, players }: {time: number, settings: Settings, players: Player[]} ) {
	...
	grantResources(): void {
		...
		for ( const team of teamKeys )
			if ( team in this.settings.resources )
				this[ team ].forEach( player => Object.assign( player.resources, this.settings.resources[ team ] ) );
	...
	updateSprites( delta: number ) {
		...
		this.sprites.forEach( sprite => {
			...
			if ( sprite.action )
			...
			else if ( Unit.isUnit( sprite ) && sprite.autoAttack && sprite.weapon ) {
				...
				const nearest = pool
					.find( u => {
						...
						if ( distanceToTarget < weapon.range + sprite.radius + u.radius )
							return true;
						...
						if ( sprite.speed ) {
							...
							if ( distance < weapon.range + sprite.radius + u.radius )
								return true;
	...
	updateResources( delta: number ) {
		...
		this.crossers.filter( p => p.unit ).forEach( player => {

			const essenseRate = this.settings.resources.crossers?.essence.rate;
